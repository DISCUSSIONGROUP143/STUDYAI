...
async function getGeminiKeys(env) {
  ...
  if (env.KV) {
    try {
      const saved = await env.KV.get('gemini:keys', 'json');
      if (Array.isArray(saved)) candidates.push(...saved);
    } catch (_) {}
  }
  const raw = String(env.GEMINI_API_KEYS || env.GEMINI_API_KEY || '');
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      if (path === '/health') return await onRequestHealth({request, env, ctx});
      if (path === '/ai') return await onRequestAI({request, env, ctx});
      if (path === '/publish-owner') return await onRequestPublish({request, env, ctx});
      if (path === '/admin/config') return await onRequestConfig({request, env, ctx});
      if (request.method === 'OPTIONS') return corsOptions(request, env);
...
// Adapt Pages Function handlers to Worker route handlers.
async function onRequestAI(args) { return await onRequestAIOriginal(args); }
async function onRequestHealth(args) { return await onRequestHealthOriginal(args); }
async function onRequestPublish(args) { return await onRequestPublishOriginal(args); }
async function onRequestConfig(args) { return await onRequestConfigOriginal(args); }
