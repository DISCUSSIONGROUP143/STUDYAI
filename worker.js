 ...
 *   ALLOWED_ORIGIN   = https://study-notes-7lw.pages.dev
 */
...
async function telegramOwnerSendMessage(env, text, reply_markup = null) {
  ...
  if (!env.TELEGRAM_LOG_CHAT_ID) {
    throw new Error('TELEGRAM_LOG_CHAT_ID is not configured.');
  }
...
async function telegramOwnerSendDocument(env, filename, content, caption) {
  ...
  form.append('chat_id', String(env.TELEGRAM_LOG_CHAT_ID));
  form.append(
    'document',
  ...
  const res = await fetch(
    `https://api.telegram.org/bot${encodeURIComponent(env.TELEGRAM_BOT_TOKEN)}/sendDocument`,
...
async function kvSavePublishMeta(env, meta) {
  // Cloudflare KV binding: variable name KV
  if (!env.KV) return {saved: false, reason: 'KV binding not configured'};
  const record = {
    ...meta,
    createdAt: new Date().toISOString()
  };
  const key = `publish:${Date.now()}:${crypto.randomUUID()}`;
  await env.KV.put(key, JSON.stringify(record));
  await env.KV.put('last_publish', JSON.stringify(record));
  return {saved: true, key};
}

async function kvGetLastPublish(env) {
  if (!env.KV) return null;
  const raw = await env.KV.get('last_publish');
  if (!raw) return null;
  try {
...
async function publishOwnerMaterial(env, body) {
  ...
  const kv = await kvSavePublishMeta(env, {
    topic,
    sourceLabel,
    questions: rawQuestions.length,
    htmlFilename: `${safe}.html`,
    jsonFilename: `${safe}.json`
  });

  return {ok: true, sent: ['message', 'html', 'json'], topic, kv};
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/health') {
      const keyText = String(env.GEMINI_API_KEYS || env.GEMINI_API_KEY || '');
      const geminiKeys = keyText.split(/[\n,]+/).map(x => x.trim()).filter(Boolean).length;
      return response({
        ok: true,
        service: 'StudyNotesAI',
        geminiKeys,
        openaiConfigured: !!env.OPENAI_API_KEY,
        telegramConfigured: !!(env.TELEGRAM_BOT_TOKEN && env.TELEGRAM_LOG_CHAT_ID),
        kvConfigured: !!env.KV,
    ...
    if (request.method === 'GET' && url.pathname === '/view') {
      ...
      if (!env.TELEGRAM_BOT_TOKEN) return new Response('Telegram is not configured', {status:500});
      const gf = await fetch(`https://api.telegram.org/bot${encodeURIComponent(env.TELEGRAM_BOT_TOKEN)}/getFile?file_id=${encodeURIComponent(fileId)}`);
      ...
      const fr = await fetch(`https://api.telegram.org/file/bot${encodeURIComponent(env.TELEGRAM_BOT_TOKEN)}/${gj.result.file_path}`);
    ...
    if (request.method === 'GET' && url.pathname === '/last-publish') {
      const last = await kvGetLastPublish(env);
      return response({ok: true, kvConfigured: !!env.KV, lastPublish: last}, 200, request, env);
    }
    ...
    if (request.method !== 'POST') {
      return response({
        ...
        endpoints: ['/health', '/ai', '/publish-owner', '/telegram-owner-test', '/last-publish', '/view?file_id=...']
      }, 200, request, env);
    }
