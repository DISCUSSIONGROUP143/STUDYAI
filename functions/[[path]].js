...
export async function onRequest(context) {
  ...
  // This is a catch-all Pages Function, so unknown/static paths must be
  // handed back to the Pages Assets service. Otherwise "/" gets swallowed
  // by this function and returns {"ok":false,"error":"Not Found"}.
  if (env.ASSETS && typeof env.ASSETS.fetch === 'function') {
    return env.ASSETS.fetch(request);
  }

  // Fallback when the Assets binding is unavailable.
