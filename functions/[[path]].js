// StudyNotesAI — Cloudflare Pages Functions backend
// Purpose: keep Telegram Bot secrets server-side and publish generated test logs.
// Gemini API is called directly by the page with a user-entered, memory-only key.
...
async function onRequestHealth({request, env}) {
...
async function onRequestPublish({request, env}) {
  if (request.method === 'OPTIONS') return corsOptions(request, env);
  ...
  if (!env.TELEGRAM_BOT_TOKEN || !getTelegramChatId(env)) {
    return json({ok:false,error:'Telegram bot token या Telegram Logs chat ID configured नहीं है।'},503,origin);
  }
  ...
  if (body.html) {
    ...
    if (!doc.ok || !dr.ok) {
      return json({ok:false,error:dr?.description || `Telegram document HTTP ${doc.status}`},502,origin);
    }
  }
...
export async function onRequest(context) {
  ...
  if (request.method === 'OPTIONS') return corsOptions(request, env);
  if (url.pathname === '/health') return onRequestHealth({request, env});
  if (url.pathname === '/publish-owner') return onRequestPublish({request, env});

  // Static files are served by Cloudflare Pages Assets.
